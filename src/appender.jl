...
mutable struct Appender
    # Keep a reference to the Julia connection, to make sure the GC does not
    ...
    function Appender(con::Connection, table::AbstractString, schema::Union{AbstractString, Nothing} = nothing)
        ...
        if duckdb_appender_create(con.handle, something(schema, C_NULL), table, handle) != DuckDBSuccess
            ...
            if isassigned(handle)
                error_data = duckdb_appender_error_data(handle[])
                error_s = unsafe_string(duckdb_error_data_message(error_data))
...
function _close_appender(appender::Appender)
    ...
    if appender.handle != C_NULL
        ...
        # anymore with duckdb_appender_error_data(). So ..._close() it first, check for error,
        ...
        if res != DuckDBSuccess
            error_message = "unknown error"
            if isassigned(appender.handle)
                error_data = duckdb_appender_error_data(appender.handle[])
                error_s = unsafe_string(duckdb_error_data_message(error_data))
                duckdb_destroy_error_data(error_data)
