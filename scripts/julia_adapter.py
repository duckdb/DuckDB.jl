...
def _render_header(version: str) -> list[str]:
...
def _render_ctypes_generated(modules: list[dict], prefix: str, resolver: _Resolver) -> list[str]:
    out = [
        "",
        "###############################################################################",
        "# ",
...
def generate(
    ...
    """Render src/api.jl and src/ctypes_generated.jl from the capigen v1 spec."""
    resolver = _Resolver(modules, metadata)
    prefix = metadata.get("prefix", "")
    version = metadata["options"]["extension"]["api_version"]

    out: list[str] = []
    out += _render_header(version)
    ...
    types = _render_ctypes_generated(modules, prefix, resolver)
